...
| legend | **배경 1 + 2N** | **항목 = 스와치 사각형 1 + 라벨 텍스트박스 1 = 2 shape**. legend 제목 필드는 v0.1 스키마에 없음 (schema.json과 일치 — 필요해지면 스키마 버전업으로 추가) |
...
## M0b 확장 필드 (eval 전용 — 골든 IR에만)

추가: 2026-07-14 (M0b 착수). 위 "M0b 확장 예약 필드" 절의 예약을 확정한다 —
`intent_align`은 행/열을 분리한 `intent_row`/`intent_col` 두 필드로 구체화되었고,
`align_hint`(런타임, M2)는 여전히 예약 상태다.

**nodes[]·containers[] 공통**으로 아래 3개 optional 필드를 허용한다:
...
- **eval 전용이며 인식 IR에는 없다.** 세션 인식이 산출하는 필드가 아니라, 골든 IR
  (`tests/fixtures/*.golden.ir.json`)에만 라벨러가 부여한다. 소비자는
  `eval/recognition_eval.py`와 `normalize/layout.py`의 `count_corrections`
  (의도-기반 과교정/미교정 카운터)뿐이다. 렌더러·정규화 본 로직은 이 필드를 읽지 않는다.
- 필드 생략은 `null`과 동치(정렬/크기 의도 없음). 그룹 라벨 문자열은 파일 내
  네임스페이스이며(관례: 행 `r1,r2,…`, 열 `c1,c2,…`, 크기 `s1,s2,…`), 한 원소가
  `intent_row`와 `intent_col`을 동시에 가질 수 있다.
- 라벨은 손그림의 **현재 좌표가 아니라 원작자의 의도**를 담는다. 부여 기준·검증 절차는
  `docs/golden-ir-labeling.md` §4 참조.
